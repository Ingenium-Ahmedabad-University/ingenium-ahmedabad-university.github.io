...
const Hero = () => {
  ...
  useEffect(()=> {
      let tl = gsap.timeline();
        tl.to(refDot.current,{css:{"bottom":"5vw"}, ease:Bounce.easeOut,duration:2.5})
        tl.to(refBox.current,{x:0,duration:2})
        tl.to(refBox.current,{yPercent:-50,height:"1.1em",duration:1.5})
        tl.to(refText.current,{css:
                                  {
  ...
  return (
    ...
    <div className="hand-text font-shadow-into-light text-center tracking-wider" ref={refHandText}>Coming Soon !</div>

    <div className="relative">
      <div className="bg-secondary absolute text-5xl md:text-8xl lg:text-9xl strike h-px w-full z-0" ref={refBox}/>
      <h1
